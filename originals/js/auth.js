...
(function () {
  ...
  window.authSignIn = async function authSignIn(e) {
    ...
    try {
      ...
      var nextRaw = new URLSearchParams(qs.replace(/^\?/, '')).get('next');
      var next = null;
      if (nextRaw) {
        if (window.dmCrypto) {
      ...
      var target = next || appHref('account/profile.html');
      setTimeout(function () { if (window.dmRouter) { dmRouter.navigate(target); } else { window.location.href = target; } }, 400);
    } catch (err) {
      console.error(err);
      showAuthMessage(friendlyAuthError(err), 'error');
    } finally {
  ...
  window.authSignOut = async function authSignOut() {
    try {
      ...
      if (window.dmRouter) { dmRouter.navigate('index.html'); } else { window.location.href = appHref('index.html'); }
    } catch (err) {
      console.error(err);
  ...
  window.initAuth = async function initAuth() {
    try {
      ...
      if (session && (onLoginPage || onRegisterPage)) {
        if (window.dmRouter) { dmRouter.navigate('account/profile.html'); return; }
