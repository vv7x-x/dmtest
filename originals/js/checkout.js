...
window.initCheckout = async function initCheckout() {
    if (cart.length === 0) {
        alert(translations[currentLang].cartEmptyAlert);
        if (window.dmRouter) { dmRouter.navigate('index.html'); return; }
...
function showCheckoutSuccess(orderId) {
    ...
    setTimeout(() => {
        ...
        var target = 'pages/order-confirmation.html?o=' + encodeURIComponent(orderParam);
        if (window.dmRouter) { dmRouter.navigate(target); } else { window.location.href = target; }
    }, 1800);
}
